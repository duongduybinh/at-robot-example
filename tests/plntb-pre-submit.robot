...
Call To Customer
    ...
    Take Screenshot
    Inject Script    ${CURDIR}/../libs/tests/plntb/pre-submit/select-filter.js
    Take Screenshot
    Inject Script    ${CURDIR}/../libs/tests/plntb/pre-submit/filter-and-select-customer.js
    Wait For Title    PL NTB - ${lastName[0]}
    Load And Init Libs
    Take Screenshot
    Inject Script    ${CURDIR}/../libs/tests/plntb/pre-submit/call-customer.js
    Take Screenshot
    ${logs}=    Get Test Logs
    Log Many    ${logs}
...
Follow Lead
    ...
    Load And Init Libs
    Take Screenshot
    Inject Script    ${CURDIR}/../libs/tests/plntb/pre-submit/change-sales-stage.js
    Take Screenshot
    Wait For Title    PL NTB - ${lastName[0]}
    ${logs}=    Get Test Logs
    Log Many    ${logs}
