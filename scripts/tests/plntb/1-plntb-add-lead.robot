...
Create a New lead on Module Lead Succesfully
    Go Page    ${HOME}index.php?module=FecLead&view=Edit&app=SALES    wait_until=load
    Load And Init Libs
    New Test Case
...
Create a PLNTB Record Succesfully
    Go Page    ${HOME}index.php?module=FecLead&view=List&app=SALES    wait_until=load
    Load And Init Libs
    New Test Case
    Take Screenshot
    ${lastName}=    Get Test Data    $.lastName
    ...
    Load And Init Libs
    Take Screenshot
    Inject Script With Retry    ${CURDIR}/../../libs/tests/plntb/add-lead/check-workflow-2.js    retries=40    timeoutMs=5000    delayBetweenRetries=3000
    Take Screenshot
    Inject Script With Retry   ${CURDIR}/../../libs/tests/plntb/add-lead/check-plntb.js    retries=20    timeoutMs=5000    delayBetweenRetries=5000
    Take Screenshot
    ${logs}=    Get Test Logs
    Log Many    ${logs}
