...
class DetailView {
    ...
    ntbEntry = {
        getNTBEntries : async () => {
            let apiUrl = Tester.detailView.getTab('NTB Entry').attr('href');
            try {
                Tester.console.log(apiUrl);
                const html = await Tester.api.getHtml(apiUrl);
                return html;
            } catch (error) {
                Tester.console.error('DetailView.getNTBEntries', error);
                return [];
            }
        },
        count: (html) => {
            ...
            if (html)
                $table = $(html).$('#listview-table');

            if ($table.hasClass('listview-table-norecords'))
                return 0;
            return $table.find('tbody tr.listViewEntries').length;
        },
        select: (entryname, appDescription, html) => {
            const $selector = $('#listview-table [data-name="fld_fecntbentryname"] a');
            if (html)
                $selector = $(html).$('#listview-table [data-name="fld_fecntbentryname"] a');
        ...
        selectAndOpen: (entryname, appDescription, html) => {
            let $select = this.ntbEntry.select(entryname, appDescription, html);
            if ($select.length == 0) {
                throw new Error(`Không tìm thấy NTB Entry(${entryname}) trong list`)
            }
    ...
    dataEntry = {
        select: (entryname, appDescription, html) => {
            const $selector = $('#listview-table [data-name="fecdataentry_dataentryname"] a');
            if (html)
                $selector = $(html).$('#listview-table [data-name="fecdataentry_dataentryname"] a');

            return $selector.filter(function () {
                if ($(this).text() === entryname) {
                    if (!appDescription)
                        return true;
                    return ($(this).closest('tr').find('[data-name="cf_fecdataentry_applicationdescription"]').text() === appDescription);
                }
                return false;
            });
        },
        selectAndOpen: (entryname, appDescription, html) => {
            let $select = this.dataEntry.select(entryname, appDescription, html);
            if ($select.length == 0) {
                throw new Error(`Không tìm thấy Data Entry (${entryname}) trong list`)
            }
