...
await Tester.asyncEvent.executeAndWait("post.click.relatedListLoad",
    ...
    async () => {
        ...
        let wf = wflogs.filter(function (w) {
            if (w.name == "[New Location] [PLNTB][NTB Entry] Submit Full Application") {
                let detail = w.details.filter(function (d) {
                    return d == "Application Status changed From Success To Queued"
                });
        ...
        if (!(wf && (wf.length > 0))) {
            throw new Error('Không tìm thấy Workflow "[New Location] [PLNTB][NTB Entry] Submit Full Application"');
        }
